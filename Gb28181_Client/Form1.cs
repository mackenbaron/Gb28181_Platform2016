...
namespace Gb28181_Client
{
    ...
    public delegate void SetCatalogText(Catalog cata,string remoteEP);
    ...
    public partial class Form1 : Form
    {
        ...
        private void Initialize()
        {
            ...
            ListViewItem lvDown = new ListViewItem(new string[] { "1", "192.168.10.134", "5060", "DownPlatform", "VERSION_2011" });
            lvPlatformList.Items.Add(lvDown);

            ListViewItem lvUp = new ListViewItem(new string[] { "2", "192.168.10.38", "5060", "UpPlatform", "VERSION_2016" });
            lvPlatformList.Items.Add(lvUp);

            SIPSqlite.Instance.Read();
            var accounts = SIPSqlite.Instance.Accounts;
            ...
            _messageDaemon = new SIPMessageDaemon(accounts, SIPRequestAuthenticator.AuthenticateSIPRequest, platformList);
        }
        ...
        private void btnStart_Click(object sender, System.EventArgs e)
        {
            ...
            _messageDaemon.MessageBase.MessageCore["192.168.10.134:5060"].OnCatalogReceived += m_msgCore_OnCatalogReceived;
        ...
        private void btnCatalog_Click(object sender, EventArgs e)
        {
            ...
            foreach (var item in lvPlatformList.SelectedItems.Cast<ListViewItem>())
            {
                devItem = item;
            }
            string key = devItem.SubItems[1].Text + ":" + devItem.SubItems[2].Text;
            _messageDaemon.MessageBase.MessageCore[key].DeviceCatalogQuery();
        }

        private void m_msgCore_OnCatalogReceived(Catalog cata,string remoteEP)
        {
            if (lvDev.InvokeRequired)
            {
                SetCatalogText setCata = new SetCatalogText(SetDevText);
                this.Invoke(setCata, cata,remoteEP);
            }
            else
            {
                SetDevText(cata,remoteEP);
            }
        }
        ...
        private void SetDevText(Catalog cata,string remoteEP)
        {
            foreach (Catalog.Item item in cata.DeviceList.Items)
            {
                ListViewItem lvItem = new ListViewItem(new string[] { _devSN.ToString(), item.Name, item.DeviceID, remoteEP });
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            c = 111 % 10;
            return;
            string sdpTxt = File.ReadAllText("D:\\sdp.txt");

            SDP sdp = SDP.ParseSDPDescription(sdpTxt);
            return;
